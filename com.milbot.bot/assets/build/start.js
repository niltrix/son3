...
function gotoMain() {
	...
	if (Utils.isNoStorage)
		for (var l in localVars)
			null != localVars[l] && (g = null == g ? l + "=" + encodeURIComponent(Utils.jsonToString(localVars[l])) : g + ("&" + l + "=" + encodeURIComponent(Utils.jsonToString(localVars[l]))));
...
$(function () {
	...
	function m(a) {
		a >= storytext.length ? ($("#story").hide(), null == Utils.getCookie("key") ? $("#create_referer_panel").show() : $("#create_referer_panel").hide(), $("#create_referer_check").attr("checked", !1), $("#divCreate").show()) : ($("#story").show(), $("#center_container").css("background-image", storytext[a].img),
			$("#storytxt").html(storytext[a].txt), a += 1, $("#story").click(function () {
				m(a);
				return !1
			}))
	}
				...
				h(b, c, function () {
					...
					if (11 == c) {
						...
						h(j + CONFIG.FUNC_LOGIN, {
							...
							if (12 == a)
								return $("#divMainMenu").hide(),
								m(0), pnlLoading.hide(), !0
						});
						return !0
					}
					return 13 == c || 14 == c ? (pnlLoading.hide(), o(), !0) : !1
				})
			}
		else
			pnlLoading.show(), b = a.server + CONFIG.FUNC_GETUSERINFO, c = {
				key : a.key
			},
...
//		}
	...
	function p(a, b, c, e, f, j) {
		...
		"undefined" != typeof syscode && null != syscode ? (window.func_temp_create_account = function (c) {
			window.func_temp_create_account = null;
			d.code = syscode;
			d.sign = c;
			pnlLoading.show();
			h(a, d, j)
		}, location.href = "js-call:makesign?func_temp_create_account&" + b + "&" + c +
				"&" + f + "&" + e) : (pnlLoading.show(), h(a, d, j))
	}
	g();
	$(window).resize(g);
	...
	$("#mod_confirm").click(function () {
		...
		if (!/^[A-Za-z0-9]+$/.test(c))
			return $("#mod_error").text(LNG.ERROR.CLIENT.INVALIDPASSWORD),
			!1;
		if (c != e)
	...
	$("#modify").click(function () {
		var a = Utils.getCookie("key");
		if (null == a)
			return $("#info p").html(LNG.ERROR.CLIENT.NOTLOGIN),
			$("#info").show(), !1;
	...
	$("#getpwd_confirm").click(function () {
		...
		b = CONFIG.MASTERHOST + CONFIG.MASTER_QUERY,
		a = {
			user : a,
			action : "login"
		};
	...
	$("#reg_confirm").click(function () {
		...
		c = "";
		if (!0 == $("#reg_referer_check").attr("checked") && (c = $("#reg_referer_on input").val(),
				"" == c))
			return $("#reg_error").text(LNG.ERROR.CLIENT.EMPTYREFERER), !1;
		pnlLoading.show();
		var e = CONFIG.MASTERHOST + CONFIG.MASTER_QUERY,
		f = {
			user : a,
			action : "reg",
			referer : c
		...
		h(e, f, function (c) {
			...
			if (13 == c) {
				var i = d.ret.user,
				i = "@" == i ? LNG.SYSTEMACCOUNT : translate(LNG.USERACCOUNT, "<b>" + i + "</b>");
	...
	$("#create_confirm").click(function () {
		...
		if (!0 == $("#create_referer_check").attr("checked") && (a = $("#create_referer_on input").val(), "" == a))
			return $("#create_error").text(LNG.ERROR.CLIENT.EMPTYREFERER),
			!1;
		pnlLoading.show();
		...
		null == b ? h(CONFIG.MASTERHOST + CONFIG.MASTER_QUERY, {
			...
			p(b + CONFIG.FUNC_REG, sysuser, syspassword, "", f, function (a) {
				...
				h(a.server + CONFIG.FUNC_CREATEROLE, {
					...
					a.ret.user && (Utils.setCookie("user",
							a.ret.user), a.ret.events ? Utils.setCookie("events", a.ret.events) : Utils.delCookie("events"), a.ret.status ? Utils.setCookie("status", a.ret.status) : Utils.delCookie("status"), Utils.setCookie("chat_sync", 1), gotoMain())
				}, function (a) {
					204 == a && k()
				})
			})
			...
			a.ret.user && (Utils.setCookie("user", a.ret.user),
				a.ret.events ? Utils.setCookie("events", a.ret.events) : Utils.delCookie("events"), a.ret.status ? Utils.setCookie("status", a.ret.status) : Utils.delCookie("status"), Utils.setCookie("chat_sync", 1), gotoMain())
		}, function (a) {
			204 == a && k()
		});
		return !1
	...
	$("#reg_referer_check").change(function () {
		!0 == $("#reg_referer_check").attr("checked") ? ($("#reg_referer_off").hide(), $("#reg_referer_on").show()) :
