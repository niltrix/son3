...
String.format = function() {
	if(0 == arguments.length)
		return null;
	for(var a = arguments[0], c = 1; c < arguments.length; c++)
		a = a.replace("{" + (c - 1) + "}", arguments[c]);
	return a
};
(function(a) {
	function c(a, c) {
	...
	a.namespace = function() {
		for(var a = arguments, c = window, b = 0, d, k; b < a.length; b++)
			if( d = a[b], d.indexOf(".")) {
				k = d.split(".");
	...
	a.Class = function(a, g) {
		...
		f.subclasses = [];
		if(a)
			b.prototype = a.prototype, f.prototype = new b, a.subclasses.push(f);
		c(f.prototype, g);
		if(!f.prototype.init)
			f.prototype.init = d;
...
(function(a) {
	a.include = function(a, b) {
		...
		for(var e = {}, g = !1, f = a.length, j = 0; j < f; j++)
			(function() {
				var b = a[j], f = b;
...
var defineSubView = ui.defineSubView, localVars = [], Utils = {
	...
	jsonToString : function(a) {
		switch(typeof a) {
			...
			case "object":
				...
				c = [];
				for(b in a)
				c.push(this.jsonToString(b) + ":" + this.jsonToString(a[b]));
				return "{" + c.join(",") + "}";
			case "number":
				return a;
			case !1:
	...
	hasLocalStorage : function() {
		try {
			if("undefined" == typeof localStorage || !window.localStorage)
				return !1
		} catch(a) {
			return !1
		}
	...
	hasSessionStorage : function() {
		try {
			if("undefined" == typeof sessionStorage || !window.sessionStorage)
				return !1
		} catch(a) {
			return !1
		}
	...
	hasStorage : function() {
		try {
			if("undefined" == typeof sessionStorage || "undefined" == typeof localStorage || !window.localStorage || !window.sessionStorage)
				return !1
		} catch(a) {
			return !1
		}
	...
	setCookie : function(a, c, b) {
		try {
			var c = this.jsonToString(c), d = new Date;
	...
	getCookie : function(a) {
		try {
			...
			if(null != c)
				return this.stringToJSON(unescape(c[2]))
		} catch(b) {
		}
		return null
	},
	...
	timeString : function(a) {
		var c = parseInt(a / 3600), b = parseInt(a % 3600 / 60);
	...
	addCss : function(a, c) {
		var b = document, d = b.getElementsByTagName("head")[0], b = b.createElement("link");
	...
	removeCss : function(a) {
		var c = document, b = c.getElementsByTagName("head")[0], a = c.getElementById(a);
	...
	cacheImage : function(a) {
		...
		if(!c)
			c = new Image, c.src = a, window.__cachedImages[a] = c;
		return c
	},
	...
	QueryString2Cookie : function() {
		...
		if(null != hu && "" != hu) {
			...
			for( i = 0; i < gy.length; i++)
				if( ft = gy[i].split("="), "undefined" != typeof ft[1] && null != ft[1] && "" != ft[1]) {
					var a = unescape(ft[1]);
					try {
...
(function(a) {
	var c = /iphone/gi.test(navigator.appVersion), b = /ipad/gi.test(navigator.appVersion), d = /android/gi.test(navigator.appVersion);
	window.isIphone = c;
	window.isAndroid = d;
	window.isIpad = b;
	jQuery.fn.touch = c || b || d ? function(a) {
	...
	if(a.hasStorage())
		a.setCookie = a._setLocal, a.getCookie = a._getLocal, a.delCookie = a._delLocal;
	else if(c || b)
		a.setCookie = a._setNoStorage, a.getCookie = a._getNoStorage, a.delCookie = a._delNoStorage, a.isNoStorage = !0;
	if(window.JSON)
		a.jsonToString = window.JSON.stringify, a.stringToJSON = window.JSON.parse
})(Utils);
...
EventManager.prototype = {
	...
	unbind : function(a, c) {
		if(a && a.constructor && a.constructor == String)
			if(c) {
	...
	trigger : function(a) {
		...
		if(a && a.constructor && a.constructor == String && ( c = this.events[a]) && c.length) {
			...
			if(1 == b)
				...
				for(; b = c[d++]; )
					b(arguments[1], arguments[2], arguments[3]);
			else if(5 == b)
				for(; b = c[d++]; )
					b(arguments[1], arguments[2], arguments[3], arguments[4]);
			else if(6 == b)
				for(; b = c[d++]; )
					b(arguments[1], arguments[2], arguments[3], arguments[4], arguments[5]);
			else if(7 == b)
				for(; b = c[d++]; )
					b(arguments[1], arguments[2], arguments[3], arguments[4], arguments[5], arguments[6]);
			else {
				for(var e = [], d = 1; d < b; d++)
					e.push(arguments[d]);
				for(; b = c[d++]; )
...
DisposeableEventManagerProxy.prototype = {
	...
	unbind : function(a, c) {
		var b = this.msgslot, d = this.msgs;
		if(c && c.constructor && c.constructor == Function) {
			for(var b = b.unbind(a, c), e = d.length; 0 <= --e; )
				d[e][0] == a && d[e][1] == c && d.splice(e, 1);
			return b
		}
		for( e = this.msgs.length; e--; )
			d[e][0] == a && (b.unbind(a, d[e][1]), d.splice(e, 1));
		return !0
	},
	dispose : function() {
		for(var a = this.msgs, c = this.msgslot, b = a.length; b--; )
			c.unbind(a[b][0], a[b][1]);
...
(function() {
	CooldownManager.prototype = {
		init : function(a, c) {
			if(this.cityid != a)
				return this.cityid = a, this.clear(), this.add(c), this
		},
		add : function(a) {
			if(a) {
		...
		tick : function() {
			for(var a = this.ema, c = this.cdlist, b = c.length, d = (new Date).getTime(); b--; ) {
				var e = c[b], g = Math.ceil((e.end - d) / 1E3);
		...
		countdown : function() {
			...
			if(0 != this.cdlist.length) {
				for(var a = this.cdlist, c = (new Date).getTime(), b = a[a.length - 1], d = a.length - 2; 0 <= d; d--)
					a[d].end < b.end && ( b = a[d]);
				a = b.end - c;
				864E5 < a && ( a = 864E5);
				0 > a && ( a = 0);
		...
		ajaxCallback : function(a) {
			...
			refreshUserInfo();
			for(var a = this.ema, b = c.length; 0 <= --b; )
				0 >= c[b].secs && c.splice(b, 1);
			var d = [], e = null, g = !1, f;
			for(f in this.cdlist) {
				e = this.cdlist[f];
				g = !1;
				for( b = c.length; b--; )
					if(c[b].id == e.id && c[b].cdtype == e.cdtype) {
						g = !0;
						break
					}
				g || d.push(e)
			...
			for( f = 0; f < d.length; )
				c = d[f], a.trigger(this.getDoneEventId(c.cdtype, c.target)), f++;
			a.trigger("endCDTimeup", d)
		},
		reload : function(a) {
			if("number" == typeof a && a != this.cityid)
				this.cityid = a, this.clear();
			this.ticktimer && clearTimeout(this.ticktimer);
			ajaxCall(this.url, {
				key : this.key,
				city : this.cityid
			}, this.ajaxCallbackDelegate, function() {
			}, !1)
		},
		getList : function(a) {
		...
		forceNotify : function(a) {
			if(this.cdlist)
				for(var c = this.ema, b = this.cdlist, d = b.length, e = (new Date).getTime(); d--; ) {
					var g = b[d], f = Math.ceil((g.end - e) / 1E3);
					if(!a || a == g.cdtype)
						0 > f && ( f = 0), c.trigger(this.getTickEventId(g.cdtype, g.target), f)
				}
		},
		changeSecs : function(a, c) {
			0 > c && ( c = 0);
			for(var b = this.cdlist.length, d = (new Date).getTime(); b--; )
				if(this.cdlist[b].id == a) {
		...
		getCD : function(a, c) {
			for(var b = this.cdlist.length; b--; )
				if(this.cdlist[b].cdtype == a && this.cdlist[b].target == c)
					return this.cdlist[b];
			return null
		}
	}
})();
(function() {
	...
	window.Debug = new a;
	$(function() {
		var a = $('<div id="debug" style="position:absolute; top:0px; right:0px; background-color:gray;font-size:12px; float:right; width:480px; height:320px;-webkit-user-select:text;-moz-user-select:text"></div>'), b = $('<div style="position:absolute;"></div>'), d = $('<div style="position:absolute;right:20px; background-color:white;"><button id="scroll">停止滚动</button><button id="close">关闭</button></div>'), e = $('<div style="position:relative; width:100%; z-index:2"></div'), g = $('<div style="position:relative;  width:100%;overflow:auto;"></div>'), f = $('<button style="position:absolute">DEBUG</button>');
		0 < window.location.href.indexOf("debug") ? (a.append(e), a.append(g), g.append(b), e.append(d), $(document.body).append(a), window.Debug.init(b, g), e = function() {
			...
			d.find("#scroll").html(window.Debug.toggle() ? "停止滚动" : "自动滚动")
...
(function() {
	if(!window.translate)
		window.translate = function() {
			for(var a = [], c = arguments, b = c[0], d = 0, e = /\%s/, g = [], f = e.exec(b); f; f = e.exec(b))
				g.push(b.substr(0, "%" == f[0][0] ? f.index : f.index)), g.push("%s"), b = b.substr(f.index + f[0].length);
			g.push(b);
			for( b = 0; b < g.length; ++b)
				( e = g[b]) && "%s" === e ? ( e = c[++d],
				void 0 === e ? a.push("%s") : a.push(e)) : a.push(e);
			return a.join("")
		}
})();
(function() {
	function a(a, c) {
		...
		if(f || this.options.desktopCompatibility)
			this.element.addEventListener(j, this, !1), this.element.addEventListener(k, this, !1), this.element.addEventListener(h, this, !1);
	...
	a.prototype = {
		...
		refresh : function() {
			...
			if(this.scrollX)
				if(0 <= this.maxScrollX)
					a = 0;
				else if(this.x < this.maxScrollX)
					a = this.maxScrollX;
			if(this.scrollY)
				if(0 <= this.maxScrollY)
					b = 0;
				else if(this.y < this.maxScrollY)
					b = this.maxScrollY;
			if(this.options.snap)
				this.maxPageX = -Math.floor(this.maxScrollX / this.scrollWidth), this.maxPageY = -Math.floor(this.maxScrollY / this.scrollHeight), b = this.snap(a, b), a = b.x, b = b.y;
			if(a != this.x || b != this.y)
				this.setTransitionTime("0"), this.setPosition(a, b, !0);
			...
			if(this.options.hScrollbar && this.scrollX)
				this.scrollBarX = this.scrollBarX || new c("horizontal", this.wrapper, this.options.fadeScrollbar, this.options.shrinkScrollbar), this.scrollBarX.init(this.scrollWidth, this.scrollerWidth);
			...
			if(this.options.vScrollbar && this.scrollY && this.scrollerHeight > this.scrollHeight)
				this.scrollBarY = this.scrollBarY || new c("vertical", this.wrapper, this.options.fadeScrollbar, this.options.shrinkScrollbar), this.scrollBarY.init(this.scrollHeight, this.scrollerHeight);
		...
		setTransitionTime : function(a) {
			...
			if(this.scrollBarX)
				this.scrollBarX.bar.style.webkitTransitionDuration = a, this.scrollBarX.wrapper.style.webkitTransitionDuration = b && this.options.fadeScrollbar ? "300ms" : "0";
			if(this.scrollBarY)
				this.scrollBarY.bar.style.webkitTransitionDuration = a, this.scrollBarY.wrapper.style.webkitTransitionDuration = b && this.options.fadeScrollbar ? "300ms" : "0"
		},
		touchStart : function(a) {
			var b;
			if(!("INPUT" == a.target.tagName || "SELECT" == a.target.tagName || "A" == a.target.tagName))
				if(f && a.preventDefault(), a.stopPropagation(), this.enabled) {
					this.scrolling = !0;
					this.moved = !1;
					this.dist = 0;
					this.setTransitionTime("0");
					if(this.options.momentum || this.options.snap)
						if( b = new WebKitCSSMatrix(window.getComputedStyle(this.element).webkitTransform), b.e != this.x || b.f != this.y)
							document.removeEventListener("webkitTransitionEnd", this, !1), this.setPosition(b.e, b.f), this.moved = !0;
		...
		touchMove : function(a) {
			var b = f ? a.changedTouches[0].pageX : a.pageX, c = f ? a.changedTouches[0].pageY : a.pageY, d = this.scrollX ? b - this.touchStartX : 0, e = this.scrollY ? c - this.touchStartY : 0, g = this.x + d, l = this.y + e;
			if(this.scrolling) {
				a.stopPropagation();
				this.touchStartX = b;
				this.touchStartY = c;
				if(0 <= g || g < this.maxScrollX)
					g = this.options.bounce ? Math.round(this.x + d / 3) : 0 <= g || 0 <= this.maxScrollX ? 0 : this.maxScrollX;
				if(0 <= l || l < this.maxScrollY)
					l = this.options.bounce ? Math.round(this.y + e / 3) : 0 <= l || 0 <= this.maxScrollY ? 0 : this.maxScrollY;
		...
		touchEnd : function(a) {
			var b = a.timeStamp - this.scrollStartTime, c = f ? a.changedTouches[0] : a, d, e;
			d = 0;
			e = this.x;
			var g = this.y;
			if(this.scrolling)
				if(this.scrolling = !1, this.moved)
					if(!this.options.snap && 250 < b)
						this.resetPosition();
					else {
						...
						if(this.options.snap)
							a = this.snap(e, g), e = a.x, g = a.y, d = Math.max(a.time, d);
					...
					if("SELECT" != d.tagName && "INPUT" != d.tagName && "TEXTAREA" != d.tagName)
						e = document.createEvent("MouseEvents"), e.initMouseEvent("click", !0, !0, a.view, 1, c.screenX, c.screenY, c.clientX, c.clientY, a.ctrlKey, a.altKey, a.shiftKey, a.metaKey, 0, null), e._fake = !0, d.dispatchEvent(e)
				}
		},
		...
		snap : function(a, b) {
			...
			if(0 < a)
				...
				this.pageX = this.maxPageX, a = this.maxScrollX;
			...
			if(0 < b)
				...
				this.pageY = this.maxPageY, b = this.maxScrollY;
		...
		scrollToPage : function(a, b, c) {
			if(!this.options.snap)
				this.pageX = -Math.round(this.x / this.scrollWidth), this.pageY = -Math.round(this.y / this.scrollHeight);
		...
		scrollToElement : function(a, b) {
			if( a = "object" == typeof a ? a : this.element.querySelector(a)) {
				...
				if(0 <= c)
					c = 0;
				else if(c < this.maxScrollX)
					c = this.maxScrollX;
				if(0 <= d)
					d = 0;
				else if(d < this.maxScrollY)
					d = this.maxScrollY;
				this.scrollTo(c, d, b)
			}
		},
		scrollToElement2 : function(a, b, c) {
			if( a = "object" == typeof a ? a : this.element.querySelector(a)) {
				var d = this.scrollX ? -a.offsetLeft : 0, a = this.scrollY ? -a.offsetTop : 0;
				if(0 > b + this.maxScrollY)
					this.maxScrollY = -b;
				if(0 <= d)
					d = 0;
		...
		momentum : function(a, b, c, d) {
			var b = 1E3 * (Math.abs(a) / b), e = b * b / 2.5 / 1E3, g = 0;
		...
		init : function(a, b) {
			...
			if("horizontal" == this.dir) {
				if(this.maxSize != this.wrapper.offsetWidth)
					this.maxSize = this.wrapper.offsetWidth, c = document.getCSSCanvasContext("2d", "scrollbar" + this.uid + this.dir, this.maxSize, 5), c.fillStyle = "rgb(0,0,0)", c.beginPath(), c.arc(2.5, 2.5, 2.5, Math.PI / 2, -Math.PI / 2, !1), c.lineTo(this.maxSize - 2.5, 0), c.arc(this.maxSize - 2.5, 2.5, 2.5, -Math.PI / 2, Math.PI / 2, !1), c.closePath(), c.fill()
			} else if(this.maxSize != this.wrapper.offsetHeight)
				this.maxSize = this.wrapper.offsetHeight, c = document.getCSSCanvasContext("2d", "scrollbar" + this.uid + this.dir, 5, this.maxSize), c.fillStyle = "rgb(0,0,0)", c.beginPath(), c.arc(2.5, 2.5, 2.5, Math.PI, 0, !1), c.lineTo(5, this.maxSize - 2.5), c.arc(2.5, this.maxSize - 2.5, 2.5, 0, Math.PI, !1), c.closePath(), c.fill();
	...
	var b = "WebKitCSSMatrix" in window && "m11" in new WebKitCSSMatrix, d = /iphone/gi.test(navigator.appVersion), e = /ipad/gi.test(navigator.appVersion), g = /android/gi.test(navigator.appVersion), f = d || e || g, j = f ? "touchstart" : "mousedown", k = f ? "touchmove" : "mousemove", h = f ? "touchend" : "mouseup", m = "translate" + ( b ? "3d(" : "("), l = b ? ",0)" : ")", n = 0;
	window.iScroll = a
})();
var Effect = {
	zoom1 : function() {
	...
	zoom : function(a, c, b, d, e, g, f) {
		var d = "number" == typeof d && 0 < d ? d : 1E3, j = (b - c) / d, k = 0 <= b - c, h = ["-webkit-transform", "-moz-transform"], m = (new Date).getTime();
		(function() {
			if(f)
				for(var b = ["-webkit-transform-origin", "-moz-transform-origin"], c = 0; c < b.length; c++)
					a.css(b[c], f.left + " " + f.top)
		})();
		(function() {
			var f;
			g && g();
	...
	SelectProxy : function() {
		function a(a) {
			...
			this.dom.panel.find("#selectproxy_panel_selected").click(function() {
				b.hidePop();
				if(b.data.newIndex != b.data.nowIndex)
					b.data.nowFace.find(".selectproxy_panel_face_text").html(b.data.nowSelect.options[b.data.newIndex].text), b.data.nowSelect.selectedIndex = b.data.newIndex, setTimeout(function() {
		...
		a.prototype = {
			...
			selectChange : function() {
				var a = this;
				if(a.data.newIndex != a.data.nowIndex || 0 > a.data.nowIndex)
					a.data.nowFace.find(".selectproxy_panel_face_text").html(a.data.nowSelect.options[a.data.newIndex].text), a.data.nowSelect.selectedIndex = a.data.newIndex, setTimeout(function() {
			...
			showPop : function(a, b) {
				function d(a, b) {
					...
					if(e.data.nowIndex == b)
						d.removeClass().addClass("selectproxy_panel_item_checked"), e.data.nowSelectBlock = d;
					/iphone/gi.test(navigator.appVersion);
					/ipad/gi.test(navigator.appVersion);
					/android/gi.test(navigator.appVersion);
					c.bind("click", function(a) {
						if(a.target == $(this).find(".selectproxy_panel_item_text")[0])
							e.data.nowSelectBlock.removeClass("selectproxy_panel_item_checked").addClass("selectproxy_panel_item_no_checked"), e.data.nowSelectBlock = d.removeClass("selectproxy_panel_item_no_checked").addClass("selectproxy_panel_item_checked"), e.data.newIndex = b, setTimeout(function() {
								e.dom.panel.hide();
								e.selectChange()
							}, 25)
					});
				...
				this.data.nowFace = b;
				for(var f = null, j = 0; j < g.length; j++)
					f = d(g[j].text, j), this.dom.move.append(f);
				f && f.css("border-bottom", "solid 0px");
				this.dom.panel.show().find("div").css("visibility", "hidden");
				setTimeout(function() {
					e.dom.panel.find("div").css("visibility", "visible");
					e.setIscroll()
				}, 250)
			},
			hidePop : function() {
			...
			proxySelect : function(a) {
				...
				if(b) {
					...
					var e = this, g = $(a), d = this.dom.template_face.clone().removeClass("template");
					d.insertBefore(g);
					0 <= a.selectedIndex ? d.find(".selectproxy_panel_face_text").html(a.options[a.selectedIndex].text) : d.find(".selectproxy_panel_face_text").html("---");
			...
			getCache : function(a, b) {
				for(var d = this.data.cache, e = 0; e < d.length; e++) {
					var g = d[e];
					if(g[0][0] == a)
						return b && d.splice(e, 1), g
				}
				return null
			},
			clearProxy : function(a) {
				if(a) {
					if( a = this.getCache(a, !0))
						a[0].show(), a[1].hide().remove()
				} else {
					for(var b = this.data.cache, d = 0; d < b.length; d++)
						a = b[d], a[0].show(), a[1].hide().remove();
					b.length = 0
				}
			}
		};
